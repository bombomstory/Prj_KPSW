...
<head>
    ...
    <style>
        ...
        .tracking-node.completed .node-dot { border-color: #198754; background: #198754; }
        .tracking-node.processing .node-dot { border-color: #0d6efd; background: #fff; animation: pulse 2s infinite; }
        .tracking-node.rejected .node-dot { border-color: #dc3545; background: #dc3545; }
    ...
    <style>
    /* บังคับใช้ฟอนต์ Kanit กับทุกองค์ประกอบในโมดูลนี้ */
    body, .modal, .menu-card, button, input, select, textarea {
        font-family: 'Kanit', sans-serif !important;
    }

    /* Git-Branch Tracking Styles */
    .tracking-wrapper { position: relative; padding: 20px 0; }
    .main-trunk { position: absolute; left: 20px; top: 0; bottom: 0; width: 3px; background: #e9ecef; z-index: 1; }
    .tracking-node { position: relative; z-index: 2; margin-bottom: 30px; padding-left: 50px; }
    .node-dot { position: absolute; left: 12px; top: 4px; width: 20px; height: 20px; border-radius: 50%; background: #fff; border: 3px solid #dee2e6; transition: all 0.3s; }
    .tracking-node.completed .node-dot { border-color: #198754; background: #198754; }
    .tracking-node.processing .node-dot { border-color: #0d6efd; background: #fff; animation: pulse 2s infinite; }
    .tracking-node.rejected .node-dot { border-color: #dc3545; background: #dc3545; }
    .branch-container { margin-top: 15px; padding-left: 20px; border-left: 2px dashed #adb5bd; margin-left: -18px; }
    .branch-item { position: relative; padding: 10px 15px; margin-bottom: 10px; background: #f8f9fa; border-radius: 8px; display: flex; justify-content: space-between; align-items: center; }
    .branch-item::before { content: ''; position: absolute; left: -20px; top: 50%; width: 20px; height: 2px; background: #adb5bd; }
    @keyframes pulse {
        0% { box-shadow: 0 0 0 0 rgba(13, 110, 253, 0.4); }
        70% { box-shadow: 0 0 0 10px rgba(13, 110, 253, 0); }
        100% { box-shadow: 0 0 0 0 rgba(13, 110, 253, 0); }
    ...
    /* สไตล์สำหรับระบบประเมินความพึงพอใจ */
...
<style>
    /* 🎨 ปรับแต่งสีและ Contrast ของแถบเมนูใน Modal (Pill Style) */
    #leaveTab {
        background-color: #e9ecef !important; /* ปรับพื้นหลังแถบรวมให้เข้มขึ้นเล็กน้อย */
        padding: 6px !important;
    }

    #leaveTab .nav-link {
        color: #495057 !important; /* เปลี่ยนจากสีเทาอ่อนเป็นสีเทาเข้มข้น มองเห็นชัดเจนแน่นอนค่ะ */
        border: none !important;
        padding: 12px 20px !important;
        transition: all 0.25s ease-in-out !important;
        font-size: 0.95rem !important;
        font-weight: 500 !important; /* เพิ่มความหนาของตัวอักษรแท็บปกติ */
    }

    /* สไตล์เมื่อแท็บถูกคลิกใช้งาน (Active) */
    #leaveTab .nav-link.active {
        background-color: #0d6efd !important; /* สีน้ำเงินหลัก */
        color: #ffffff !important; /* ตัวหนังสือสีขาวเด่น */
        font-weight: 600 !important;
        box-shadow: 0 4px 10px rgba(13, 110, 253, 0.3) !important;
    }

    /* สไตล์เมื่อเอาเมาส์ไปชี้แท็บที่ยังไม่ได้คลิก (Hover) */
    #leaveTab .nav-link:hover:not(.active) {
        background-color: #dee2e6 !important; /* ไฮไลท์พื้นหลังให้อ่อนๆ */
        color: #0d6efd !important; /* เปลี่ยนสีตัวหนังสือและไอคอนเป็นสีน้ำเงินตามธีม */
    }

    /* บังคับใช้ฟอนต์ Kanit ให้สวยงามสม่ำเสมอ */
    .nav-link, .modal-title, .fw-bold {
        font-family: 'Kanit', sans-serif !important;
    }
</style>
...
<body>
    ...
    <section id="dashboard" class="container">
        <div class="dashboard-container fade-in">
            ...
            <div class="menu-grid">
                ...
                <div class="menu-card slide-up" onclick="showLeaveModal()">
                    <div class="menu-icon">
                        <i class="bi bi-envelope-paper"></i>
                    </div>
                    <h5 class="menu-title">ยื่นใบลาออนไลน์</h5>
                    <p class="menu-description">ยื่นใบลาป่วยหรือลากิจ พร้อมติดตามสถานะการอนุมัติแบบเรียลไทม์</p>
                </div>
            </div>
        </div>
    </section>
    ...
    // --- 1. Query ค้นหาข้อมูลใบลาล่าสุดเพื่อแสดงผลในแท็บ "ติดตามสถานะ" ---
    $sql_status = "SELECT * FROM leave_requests WHERE student_id = ? ORDER BY leave_id DESC LIMIT 1";
    $stmt_status = $conn->prepare($sql_status);
    $stmt_status->bind_param("i", $session_student_id);
    $stmt_status->execute();
    $res_status = $stmt_status->get_result();
    $latest_leave = $res_status->fetch_assoc();
    $stmt_status->close();

    // --- 2. Query ค้นหาใบลาที่สำเร็จแล้วแต่ยังไม่เคยทำประเมิน เพื่อแสดงในแท็บ "ประเมินระบบ" ---
    $sql_un評 = "SELECT lr.leave_id, lr.leave_type, lr.leave_date 
                FROM leave_requests lr 
                LEFT JOIN leave_satisfaction ls ON lr.leave_id = ls.leave_id 
                WHERE lr.student_id = ? AND lr.overall_status = 'completed' AND ls.eval_id IS NULL 
                LIMIT 1";
    $stmt_un評 = $conn->prepare($sql_un評);
    $stmt_un評 = $conn->prepare($sql_un評);
    $stmt_un評->bind_param("i", $session_student_id);
    $stmt_un評->execute();
    $res_un評 = $stmt_un評->get_result();
    $pending_eval = $res_un評->fetch_assoc();
    $stmt_un評->close();
    ?>
    ...
    <style>
        ...
        #leaveModal, #leaveModal *, .modal-title, .nav-link, .form-label, .badge {
            font-family: 'Kanit', sans-serif !important;
        }
    </style>
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header border-0 pb-0">
                    <h5 class="modal-title fw-bold text-dark" id="leaveModalLabel">
                        <i class="bi bi-envelope-paper-fill text-primary me-2"></i>ระบบใบลาออนไลน์
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                
                <div class="modal-body p-4">
                    <ul class="nav nav-pills nav-justified mb-4 p-1 bg-light rounded-pill" id="leaveTab" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active rounded-pill fw-bold" id="new-leave-tab" data-bs-toggle="tab" data-bs-target="#new-leave" type="button">
                                <i class="bi bi-pencil-square me-2"></i>ยื่นใบลาใหม่
                            </button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link rounded-pill fw-bold" id="status-tab" data-bs-toggle="tab" data-bs-target="#status" type="button">
                                <i class="bi bi-diagram-3-fill me-2"></i>ติดตามสถานะ
                            </button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link rounded-pill fw-bold" id="eval-tab" data-bs-toggle="tab" data-bs-target="#eval" type="button">
                                <i class="bi bi-chat-left-heart-fill me-2"></i>ประเมินระบบ
                            </button>
                        </li>
                    </ul>

                    <div class="tab-content">
                        <div class="tab-pane fade show active" id="new-leave">
                            <form id="insertLeaveForm">
                                <div class="row g-3">
                                    <div class="col-md-6">
                                        <label class="form-label small fw-bold">ประเภทการลา</label>
                                        <select class="form-select border-0 bg-light" name="leave_type" required>
                                            <option value="ลาป่วย">ลาป่วย (Sick Leave)</option>
                                            <option value="ลากิจ">ลากิจ (Business Leave)</option>
                                        </select>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label small fw-bold">วันที่ต้องการลา</label>
                                        <input type="date" class="form-control border-0 bg-light" name="leave_date" required>
                                    </div>
                                    <div class="col-12">
                                        <label class="form-label small fw-bold">เหตุผลการลา</label>
                                        <textarea class="form-control border-0 bg-light" name="leave_reason" rows="4" placeholder="โปรดระบุรายละเอียดเหตุผลการลา..." required></textarea>
                                    </div>
                                    <div class="col-12 text-end mt-4">
                                        <button type="submit" class="btn btn-primary px-4 py-2 rounded-pill fw-bold">ส่งใบลาออนไลน์</button>
                                    </div>
                                </div>
                            </form>
                        </div>
                        ...
                        <div class="tab-pane fade" id="status">
                            <div class="bg-white p-2 rounded">
                                <?php
                                // ดึงประวัติการลาทั้งหมดของนักเรียนปัจจุบัน เรียงจากล่าสุดลงไป
                                $sql_semester_leaves = "SELECT * FROM leave_requests WHERE student_id = ? ORDER BY leave_id DESC";
                                $stmt_sem = $conn->prepare($sql_semester_leaves);
                                $stmt_sem->bind_param("i", $session_student_id);
                                $stmt_sem->execute();
                                $res_sem = $stmt_sem->get_result();
                                
                                if ($res_sem->num_rows > 0):
                                ?>
                                    <div id="leave-history-list-view">
                                        <div class="d-flex align-items-center mb-3">
                                            <h6 class="fw-bold text-dark mb-0">
                                                <i class="bi bi-journal-text text-primary me-2"></i>ประวัติการยื่นใบลาในภาคเรียนนี้
                                            </h6>
                                            <span class="badge bg-secondary ms-2 rounded-pill"><?php echo $res_sem->num_rows; ?> รายการ</span>
                                        </div>
                                        
                                        <div class="row g-3">
                                            <?php 
                                            while ($leave_row = $res_sem->fetch_assoc()): 
                                                // คัดกรองและปรับแต่ง Badge สถานะหลัก
                                                $status_title = 'กำลังดำเนินการ'; $badge_style = 'bg-primary';
                                                if($leave_row['overall_status'] == 'completed') { $status_title = 'อนุมัติเสร็จสิ้น'; $badge_style = 'bg-success'; }
                                                if($leave_row['overall_status'] == 'rejected') { $status_title = 'ปฏิเสธคำร้อง'; $badge_style = 'bg-danger'; }
                                            ?>
                                                <div class="col-12">
                                                    <div class="card border-0 bg-light shadow-sm hover-shadow transition-all" style="border-radius: 15px;">
                                                        <div class="card-body p-3">
                                                            <div class="d-flex justify-content-between align-items-start mb-2">
                                                                <div>
                                                                    <span class="badge <?php echo $badge_style; ?> px-2 py-1 rounded-pill small mb-2">
                                                                        <?php echo $status_title; ?>
                                                                    </span>
                                                                    <h6 class="fw-bold text-dark mb-1">ใบ<?php echo $leave_row['leave_type']; ?></h6>
                                                                </div>
                                                                <small class="text-muted fw-bold">
                                                                    <i class="bi bi-calendar3 me-1"></i><?php echo date('d M Y', strtotime($leave_row['leave_date'])); ?>
                                                                </small>
                                                            </div>
                                                            
                                                            <p class="card-text text-secondary small mb-3 text-truncate-2" style="font-size: 0.85rem;">
                                                                <strong>เหตุผล:</strong> <?php echo htmlspecialchars($leave_row['leave_reason']); ?>
                                                            </p>
                                                            
                                                            <div class="text-end">
                                                                <button type="button" class="btn btn-outline-primary btn-sm rounded-pill px-3 fw-bold shadow-sm"
                                                                        onclick="viewSingleTimeline(<?php echo $leave_row['leave_id']; ?>, '<?php echo $leave_row['leave_type']; ?>', '<?php echo date('d M Y', strtotime($leave_row['leave_date'])); ?>')">
                                                                    <i class="bi bi-eye-fill me-1"></i> ติดตามสถานะการอนุมัติ
                                                                </button>
                                                            </div>
                                                        </div>
                                                    </div>
                                    ...
                                    <div id="leave-single-timeline-view" style="display: none;">
                                        <button type="button" class="btn btn-light btn-sm rounded-pill px-3 mb-3 border fw-bold text-secondary" onclick="backToHistoryList()">
                                            <i class="bi bi-arrow-left me-1"></i> ย้อนกลับไปประวัติทั้งหมด
                                        </button>
                                        
                                        <div class="p-3 bg-light rounded-3 border-0 mb-4" style="border-radius: 15px;">
                                            <div class="d-flex justify-content-between align-items-center">
                                                <h6 class="fw-bold mb-0 text-dark"><i class="bi bi-search me-1"></i> ผลการตรวจสอบ: <span id="target-leave-title" class="text-primary"></span></h6>
                                                <small id="target-leave-date" class="text-muted fw-bold"></small>
                                            </div>
                                        </div>

                                        <?php
                                        // ทำการวนลูป Query ดึงกล่องไทม์ไลน์ออกมาเตรียมไว้ในระบบหลังบ้าน
                                        $res_sem->data_seek(0);
                                        while ($timeline_row = $res_sem->fetch_assoc()):
                                            $parent_ok = ($timeline_row['parent_status'] == 'approved');
                                            $advisor_ok = ($timeline_row['advisor_status'] == 'approved');
                                        ?>
                                            <div class="single-timeline-box" id="detail-box-<?php echo $timeline_row['leave_id']; ?>" style="display: none;">
                                                <div class="tracking-wrapper" style="position: relative; padding: 10px 0;">
                                                    <div class="main-trunk"></div>

                                                    <div class="tracking-node completed">
                                                        <div class="node-dot"></div>
                                                        <div class="node-content">
                                                            <h6 class="mb-0 fw-bold text-success">นักเรียนยื่นคำร้องใบลาสำเร็จ</h6>
                                                            <p class="text-muted small mb-0">ระบบบันทึกเข้าระบบส่วนกลางเมื่อ <?php echo date('d/m/Y H:i', strtotime($timeline_row['created_at'])); ?> น.</p>
                                                        </div>
                                                    </div>

                                                    <div class="tracking-node <?php echo $parent_ok ? 'completed' : 'processing'; ?>">
                                                        <div class="node-dot"></div>
                                                        <div class="node-content">
                                                            <h6 class="mb-0 fw-bold <?php echo $parent_ok ? 'text-success' : 'text-primary'; ?>">ผู้ปกครองกดรับรองความถูกต้อง</h6>
                                                            <p class="text-muted small mb-0"><?php echo $parent_ok ? 'ผู้ปกครองทำการเซ็นดิจิทัลรับรองเรียบร้อยแล้วค่ะ' : 'กำลังรอระบบแจ้งเตือนและรับรองจากผู้ปกครอง'; ?></p>
                                                        </div>
                                                    </div>

                                                    <div class="tracking-node <?php echo $advisor_ok ? 'completed' : ($parent_ok ? 'processing' : ''); ?>">
                                                        <div class="node-dot"></div>
                                                        <div class="node-content">
                                                            <h6 class="mb-0 fw-bold <?php echo $advisor_ok ? 'text-success' : 'text-secondary'; ?>">ครูที่ปรึกษาอนุมัติผ่านด่านหลัก</h6>
                                                            <p class="text-muted small mb-0"><?php echo $advisor_ok ? 'ครูที่ปรึกษาพิจารณาหลักฐานและกดอนุมัติแล้ว' : 'คำร้องอยู่ในคิวรอการตรวจสอบจากครูที่ปรึกษา'; ?></p>
                                                        </div>

                                                        <?php if ($advisor_ok): ?>
                                                            <div class="branch-container mt-3 ps-3 border-start border-2 border-light">
                                                                <div class="small fw-bold text-secondary mb-2"><i class="bi bi-diagram-3 me-1"></i> รายวิชาในวันที่ลาที่แจ้งระบบ Attendance อัตโนมัติ</div>
                                                                
                                                                <?php
                                                                $sql_br = "SELECT * FROM leave_subject_branches WHERE leave_id = ?";
                                                                $stmt_b = $conn->prepare($sql_br);
                                                                $stmt_b->bind_param("i", $timeline_row['leave_id']);
                                                                $stmt_b->execute();
                                                                $res_b = $stmt_b->get_result();
                                                                
                                                                while ($b_item = $res_b->fetch_assoc()):
                                                                    $b_border = 'border-warning'; $b_badge = 'bg-warning text-dark'; $b_txt = 'รอการตรวจสอบ';
                                                                    if($b_item['teacher_status'] == 'acknowledged') { $b_border = 'border-success'; $b_badge = 'bg-success text-white'; $b_txt = 'รับทราบแล้ว'; }
                                                                    if($b_item['teacher_status'] == 'rejected') { $b_border = 'border-danger'; $b_badge = 'bg-danger text-white'; $b_txt = 'ปฏิเสธการลา'; }
                                                                ?>
                                                                    <div class="branch-item shadow-sm border-start <?php echo $b_border; ?> border-4 mb-2 p-2 bg-light rounded d-flex justify-content-between align-items-center">
                                                                        <div>
                                                                            <div class="fw-bold small text-dark"><?php echo htmlspecialchars($b_item['subject_name']); ?></div>
                                                                            <small class="text-muted" style="font-size: 0.7rem;">อาจารย์ผู้สอน: <?php echo htmlspecialchars($b_item['teacher_name']); ?></small>
                                                                        </div>
                                                                        <span class="badge <?php echo $b_badge; ?> rounded-pill px-3" style="font-size: 0.7rem;"><?php echo $b_txt; ?></span>
                                                                    </div>
                                                                <?php endwhile; $stmt_b->close(); ?>
                                                            </div>
                                                        </div>
                                                    <?php endif; ?>
                                                </div>
                                ...
                                <?php else: ?>
                                    <div class="text-center p-5">
                                        ...
                                        <h6 class="mt-3 fw-bold text-secondary">ยังไม่มีข้อมูลประวัติการลาในภาคเรียนนี้ค่ะ</h6>
                                    </div>
                                <?php endif; $stmt_sem->close(); ?>
                            </div>
                        </div>
                        ...
                        <div class="tab-pane fade" id="eval">
                            <?php if ($pending_eval): ?>
                                <form id="satisfactionForm">
                                    <input type="hidden" name="leave_id" value="<?php echo $pending_eval['leave_id']; ?>">
                                    
                                    <div class="alert alert-info border-0 rounded-3 small mb-4">
                                        <i class="bi bi-info-circle-fill me-2"></i> พบรายการคำร้องพิจารณาสำเร็จในรายการ ใบ<?php echo $pending_eval['leave_type']; ?> วันที่ <?php echo date('d M Y', strtotime($pending_eval['leave_date'])); ?> โปรดช่วยสละเวลาให้คะแนนเพื่อพัฒนาระบบค่ะ
                                    </div>

                                    <div class="mb-4">
                                        <label class="form-label small fw-bold text-dark d-block mb-2">1. ความรวดเร็วในการประสานงานและการอนุมัติใบลา (SLA Performance)</label>
                                        <div class="star-rating">
                                            <input type="radio" id="speed-5" name="rating_speed" value="5" required /><label for="speed-5"><i class="bi bi-star-fill"></i></label>
                                            <input type="radio" id="speed-4" name="rating_speed" value="4" /><label for="speed-4"><i class="bi bi-star-fill"></i></label>
                                            <input type="radio" id="speed-3" name="rating_speed" value="3" /><label for="speed-3"><i class="bi bi-star-fill"></i></label>
                                            <input type="radio" id="speed-2" name="rating_speed" value="2" /><label for="speed-2"><i class="bi bi-star-fill"></i></label>
                                            <input type="radio" id="speed-1" name="rating_speed" value="1" /><label for="speed-1"><i class="bi bi-star-fill"></i></label>
                                        </div>
                                    </div>

                                    <div class="mb-4">
                                        <label class="form-label small fw-bold text-dark d-block mb-2">2. ความง่ายและความสะดวกรวดเร็วในการกรอกและติดตามฟลูงาน</label>
                                        <div class="star-rating">
                                            <input type="radio" id="easy-5" name="rating_easiness" value="5" required /><label for="easy-5"><i class="bi bi-star-fill"></i></label>
                                            <input type="radio" id="easy-4" name="rating_easiness" value="4" /><label for="easy-4"><i class="bi bi-star-fill"></i></label>
                                            <input type="radio" id="easy-3" name="rating_easiness" value="3" /><label for="easy-3"><i class="bi bi-star-fill"></i></label>
                                            <input type="radio" id="easy-2" name="rating_easiness" value="2" /><label for="easy-2"><i class="bi bi-star-fill"></i></label>
                                            <input type="radio" id="easy-1" name="rating_easiness" value="1" /><label for="easy-1"><i class="bi bi-star-fill"></i></label>
                                        </div>
                                    </div>

                                    <div class="mb-4">
                                        <label class="form-label small fw-bold text-dark mb-2">ข้อเสนอแนะเพิ่มเติมเพื่อการพัฒนาปรับปรุงระบบ</label>
                                        <textarea class="form-control border-0 bg-light" name="feedback" rows="3" placeholder="ระบุสิ่งที่อยากให้ปรับปรุงเพิ่มเติม..."></textarea>
                                    </div>

                                    <div class="text-end">
                                        <button type="submit" class="btn btn-success px-4 py-2 rounded-pill fw-bold">บันทึกผลการประเมิน</button>
                                    </div>
                                </form>
                            <?php else: ?>
                                <div class="text-center p-4">
                                    <i class="bi bi-check-circle-fill text-success display-4"></i>
                                    <p class="mt-2 text-secondary">ไม่มีรายการใบลาค้างประเมินความพึงพอใจในขณะนี้ค่ะ ขอบคุณค่ะ</p>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
    ...
    <div class="tab-pane fade" id="status">
        ...
        <div id="feedbackSection" class="mt-4 p-3 bg-light rounded d-none">
            <h6 class="fw-bold">ประเมินระบบการลา</h6>
            <form id="feedbackForm">
                <select name="score" class="form-select mb-2"><option value="5">⭐⭐⭐⭐⭐</option><option value="1">⭐</option></select>
                <textarea name="comment" class="form-control" placeholder="ข้อเสนอแนะ"></textarea>
                <button type="submit" class="btn btn-sm btn-success mt-2">ส่งการประเมิน</button>
            </form>
        </div>
    </div>
    ...
    // ฟังก์ชันสำหรับเปิดหน้าแสดงผล Timeline รายฉบับ
    function viewSingleTimeline(leaveId, leaveType, leaveDate) {
        // 1. นำข้อมูลหัวข้อและวันที่ไปใส่ในกล่อง Header
        document.getElementById('target-leave-title').innerText = "ใบ" + leaveType;
        document.getElementById('target-leave-date').innerHTML = "<i class='bi bi-calendar-check me-1'></i>วันที่ลา: " + leaveDate;
        
        // 2. ซ่อนหน้าประวัติภาพรวมทั้งหมด
        document.getElementById('leave-history-list-view').style.display = 'none';
        
        // 3. ซ่อนกล่องไทม์ไลน์อันอื่นที่อาจค้างอยู่ และเปิดเฉพาะกล่อง leaveId ที่เลือก
        document.querySelectorAll('.single-timeline-box').forEach(box => box.style.display = 'none');
        var selectedBox = document.getElementById('detail-box-' + leaveId);
        if(selectedBox) {
            selectedBox.style.display = 'block';
        }
        
        // 4. เปิดม่านแสดงผลหน้าไทม์ไลน์เดียว
        document.getElementById('leave-single-timeline-view').style.display = 'block';
    }

    // ฟังก์ชันกดย้อนกลับมาดูประวัติการลาทั้งหมดในภาคเรียน
    function backToHistoryList() {
        // ซ่อนหน้าไทม์ไลน์ฉบับเดียว แล้วเปิดหน้าประวัติภาพรวมคืนกลับมาค่ะ
        document.getElementById('leave-single-timeline-view').style.display = 'none';
        document.getElementById('leave-history-list-view').style.display = 'block';
    }

    // ฟังก์ชันเรียกดูสถานะ
    function loadTracking(leave_id) {
        fetch('get_leave_status.php?leave_id=' + leave_id)
        .then(response => response.json())
        .then(data => {
            // อัปเดต UI ตามสถานะ (Git-Branch logic)
    ...
    <?php
        ...
        $student_classroom = getStudentClassroom($_SESSION['user_id']);
    ?>
        ...
        // ระบบบันทึกแบบประเมินความพึงพอใจผ่าน AJAX ด้วย Pure PHP และ MySQLi หลังบ้าน
        // ==========================================
        // ระบบส่งฟอร์มใบลาออนไลน์พร้อมป๊อบอัพแจ้งเตือน
        // ==========================================
        document.getElementById('insertLeaveForm').addEventListener('submit', function(e) {
            e.preventDefault(); // ป้องกันไม่ให้หน้าจอรีเฟรชก่อนบันทึกสำเร็จ
            
            // ดึงข้อมูลจากฟอร์มใบลา
            var formData = new FormData(this);
            
            // ส่งข้อมูลไปประมวลผลที่หลังบ้าน (พี่ทูลสามารถสร้างไฟล์สำหรับรับค่าฟอร์ม หรือโยนให้ไฟล์ที่มีอยู่แล้วจัดการได้ค่ะ)
            fetch('save_leave_request.php', {
                method: 'POST',
                body: formData
            })
            .then(response => response.json())
            .then(data => {
                if(data.status === 'success') {
                    // เด้งป๊อบอัพแจ้งเตือนสำเร็จ และรอให้เด็กกด OK ก่อน
                    alert("บันทึกข้อมูลใบลาสำเร็จแล้วค่ะ 👌");
                    
                    // หลังจากกด OK ให้ทำการรีเฟรชหน้าจอเพื่ออัปเดตข้อมูลบนหน้า Dashboard และ Timeline ทันที
                    location.reload();
                } else {
                    // กรณีเกิดข้อผิดพลาดจากระบบหลังบ้าน
                    alert("เกิดข้อผิดพลาด: " + data.message);
                }
            })
            .catch(error => {
                console.error('Error:', error);
                alert("ไม่สามารถติดต่อเซิร์ฟเวอร์ได้ในขณะนี้ กรุณาลองใหม่อีกครั้งค่ะ");
            });
        });
